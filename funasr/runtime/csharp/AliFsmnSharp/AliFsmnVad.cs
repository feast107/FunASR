...
public class AliFsmnVad {
    ...
    public SegmentEntity[] GetSegments(List<float[]> samples) {
        ...
        for (int beg_idx = 0; beg_idx < waveform_nums; beg_idx += _batchSize) {
            ...
            List<VadInputEntity> vadInputEntities = ExtractFeats(waveform_list);
            try {
                int t_offset = 0;
                int step = Math.Min(waveform_list.Max(x => x.Length), 6000);
                bool is_final = true;
                List<VadOutputEntity> vadOutputEntities = Infer(vadInputEntities);
                for (int batch_num = beg_idx; batch_num < end_idx; batch_num++) {
                    var scores = vadOutputEntities[batch_num - beg_idx].Scores;
                    SegmentEntity[] segments_part = vadInputEntities[batch_num].VadScorer.DefaultCall(scores,
                        waveform_list[batch_num - beg_idx], is_final: is_final, max_end_sil: _max_end_sil,
                        online: false);
...
//                }
    ...
    public SegmentEntity[] GetSegmentsByStep(List<float[]> samples) {
        ...
        for (int beg_idx = 0; beg_idx < waveform_nums; beg_idx += _batchSize) {
            ...
            List<VadInputEntity> vadInputEntities = ExtractFeats(waveform_list);
            int feats_len = vadInputEntities.Max(x => x.SpeechLength);
            List<float[]> in_cache = new List<float[]>();
            in_cache = PrepareCache(in_cache);
            try {
                int step = Math.Min(vadInputEntities.Max(x => x.SpeechLength), 6000 * 400);
                bool is_final = true;
                for (int t_offset = 0; t_offset < (int)(feats_len); t_offset += Math.Min(step, feats_len - t_offset)) {
                    ...
                    List<VadInputEntity> vadInputEntities_step = new List<VadInputEntity>();
                    foreach (VadInputEntity vadInputEntity in vadInputEntities) {
                        ...
                        if (curr_step <= 0) {
                            ...
                            vadInputEntities_step.Add(vadInputEntity_step);
                            continue;
                        }
                        ...
                        float[]? waveform_step =
                            new float[Math.Min(waveform.Length, ((int)(t_offset + step) / 400 - 1) * 160 + 400) -
                                      t_offset / 400 * 160];
                    ...
                    List<VadOutputEntity> vadOutputEntities = Infer(vadInputEntities_step);
                    for (int batch_num = 0; batch_num < _batchSize; batch_num++) {
                        vadInputEntities[batch_num].InCaches = vadOutputEntities[batch_num].OutCaches;
                        var scores = vadOutputEntities[batch_num].Scores;
                        SegmentEntity[] segments_part = vadInputEntities[batch_num].VadScorer.DefaultCall(scores,
                            vadInputEntities_step[batch_num].Waveform, is_final: is_final, max_end_sil: _max_end_sil,
                            online: false);
                        if (segments_part.Length > 0) {
#pragma warning disable CS8602 // 解引用可能出现空引用。
...
#pragma warning restore CS8602 // 解引用可能出现空引用。
    ...
    private List<VadInputEntity> ExtractFeats(List<float[]> waveformList) {
        var inCaches = new List<float[]>();
        inCaches = PrepareCache(inCaches);
        var vadInputEntities = new List<VadInputEntity>();
        foreach (var waveform in waveformList) {
            var fbank = _wavFrontend.GetFbank(waveform);
            var features = _wavFrontend.LfrCmvn(fbank);
            var vadInputEntity = new VadInputEntity {
                Waveform = waveform,
                Speech = features.SelectMany(x => x).ToArray(),
                SpeechLength = features.Length,
                InCaches = inCaches,
                VadScorer = new E2EVadModel(_vad_post_conf)
            };

            vadInputEntities.Add(vadInputEntity);
        }

        return vadInputEntities;
    }
    ...
    /// <returns></returns>
    private static T[,,] DimOneToThree<T>(T[] obj, int len, int wid) {
        if (obj.Length % (len * wid) != 0)
            return null;
        
        var height = obj.Length / (len * wid);
        var threeDimObj = new T[len, wid, height];
    ...
    private List<VadOutputEntity> Infer(List<VadInputEntity> vadInputEntities) {
        var vadOutputEntities = new List<VadOutputEntity>();
        foreach (var vadInputEntity in vadInputEntities) {
...
#pragma warning restore CS8602 // 解引用可能出现空引用。
            ...
            var arr_inv_stddev = _onnxSession.ModelMetadata.CustomMetadataMap["inv_stddev"].ToString().Split(',')
                .ToArray();
